...
def get_ssh_statistics(
    ...
    for point in error_results[0]:
        ...
        stats["recent_errors"].append({
            "host": payload.get("host", ""),
            "command": payload.get("command", ""),
            "error": payload.get("log_line", ""),
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(payload.get("timestamp", 0)))
        })
    
    # Sort most common items
    if host_filter is None:
        stats["most_used_hosts"] = dict(sorted(stats["most_used_hosts"].items(), key=lambda x: x[1], reverse=True)[:10])
       
    stats["most_common_commands"] = dict(sorted(stats["most_common_commands"].items(), key=lambda x: x[1], reverse=True)[:10])
...
def suggest_commands(
    context: Annotated[str, "Current situation or goal (e.g. 'check disk space', 'restart service')"],
    ...
    filters = [FieldCondition(key="return_code", match={"value": 0})]  # Only successful commands
    if host:
    ...
    for point in results.points:
        ...
        if command and command not in suggestions:
            suggestions[command] = {
                ...
                "relevance_score": point.score,
                "host": payload.get("host", ""),
                "last_used": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(payload.get("timestamp", 0))),
                "success_rate": 1.0  # We only queried successful commands
            }
    
    # Sort by relevance and limit results
    sorted_suggestions = sorted(suggestions.values(), key=lambda x: x["relevance_score"], reverse=True)[:limit]
